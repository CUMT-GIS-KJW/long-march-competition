(function () {
    ...
    const values =
      activeTool === "node"
        ? countBy(data.events, "type").map((item) => ({
            name: MapUtils.typeNames[item.name],
            value: item.value,
          }))
  ...
  function updateConclusion() {
    const conclusions = {
      route: "路线统计显示，四川、贵州段里程与节点密度较高，是长征历史叙事与资源展示的重点区域。",
      terrain: `路线平均海拔约 ${data.summary.averageElevation} m，最高点约 ${data.summary.maxElevation} m，川西雪山草地段地形阻隔最显著。`,
      buffer: `${$("#bufferSelect").value} km 缓冲范围用于统计沿线历史节点、红色资源数量与覆盖面积。`,
      node: "节点类型以战役战斗和会议会师为主，重要节点沿中央红军主线形成明显时空序列。",
      resource: "红色资源与遵义、泸定、会宁等关键节点高度耦合，具备构建主题研学线路的空间基础。",
    };
