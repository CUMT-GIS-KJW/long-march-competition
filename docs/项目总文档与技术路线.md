...
| 红色研学组织者 | 设计研学路线和现场教学方案 | 红色旅游资源筛选、研学路线生成、知识图谱 |
...
系统默认访问 `/` 时进入登录页，登录状态保存在浏览器本地存储中。登录后可进入首页主地图，并通过页面导航访问综合分析、红色研学和三维场景。
...
| `/api/red-tourism/resources` | GET | 红色旅游资源数据 |
...
推荐演示顺序：登录进入首页，展示长征路线总体分布；打开路线图层，演示多部队路线分层；播放中央红军路线动画，讲解时间线、节点和详情卡片联动；切换到综合分析页，展示路线统计、地形剖面、缓冲区和资源关联；从遵义会议节点进入三维场景，展示 Cesium 地形、核心点和测距交互；进入红色研学页，按主题生成一条研学路线方案；最后打开 AI 助手，演示对当前分析结果的解释能力。
