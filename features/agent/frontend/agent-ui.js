(function () {
  ...
  function analysisAdvice(topic) {
    ...
    if (topic.includes("资源")) {
      return "红色资源关联分析重点说明沿线资源热点与历史事件的耦合关系，可支撑红色研学线路设计。";
    }
  ...
  function buildComprehensiveAnalysis(results, context) {
    ...
    return [
      ...
      "",
      `六、当前页面专题：当前选中“${context?.topic || "综合分析"}”，路线为“${context?.selectedRoute || "当前路线"}”。如果用于答辩，可以概括为：本系统以 GIS 分析结果为依据，将长征路线的空间跨度、地形阻力、事件节点和红色资源联系起来，证明长征精神既有历史叙事价值，也有现实研学组织价值。`,
    ].join("\n");
  }
