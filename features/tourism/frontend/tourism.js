(function () {
  ...
  const featuredMapIllustrations = [
    ...
    { province: "四川省", kind: "panda", lat: 31.70, lng: 97.40, leader: { lat: 30.45, lng: 103.15 }, size: 106, rotate: -4, shift: "sichuan" },
  ...
  function renderProvinceTourismMap(resources = state.resources) {
  ...
  async function generateRoute() {
    ...
    renderProvinceTourismMap(route.provinceResources || route.resources);
    renderResourceMarkers(
    ...
    $("#routePanel").innerHTML = `
      ...
      <div class="route-summary">
        ...
        <div class="route-tags">
          ...
          <span>省内：${route.province || route.resources[0]?.province || "自动匹配"}</span>
  ...
  function showArcRoute(route) {
    ...
    for (let index = 0; index < resources.length - 1; index += 1) {
      ...
      L.marker(middle, {
        icon: L.divIcon({
          className: "",
          html: `<div class="segment-badge" style="--day-color:${group.color}">D${group.day || 1}</div>`,
          iconSize: [34, 22],
          iconAnchor: [17, 11],
        }),
        interactive: false,
  ...
  function bindRoutePresetCards() {
    document.querySelectorAll("[data-route-preset]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const province = card.dataset.presetProvince || "";
        const theme = card.dataset.presetTheme || "meeting";
        const days = card.dataset.presetDays || "3";
